...
describe('Math', () => {
  ...
  describe('random', () => {
    it('should return a random number between min/max', () => {
      ...
      expect(randomNumber).to.be.a.float;
    });
  });
  ...
  describe('getDegreeOfPoints', () => {
    it('should return the expected degree of our given points', () => {
      expect(Math.getDegreeOfPoints(10, 10, 5, 5)).to.be.equal(45);
    });
  });

  describe('setPointOnEllipse', () => {
    expect('should set x, y to an object based off of the ellipse points', () => {
      const reusedObject = {
        x: 0,
        y: 0
      };
      ...
      expect(reusedObject.x).to.be.equal(10);
      expect(reusedObject.y).to.be.equal(10);
    });
  });

  describe('getPointOnQuadraticCurve', () => {
    it ('should give us a point based off of time along a quadratic curve', () => {
      expect(Math.getPointOnQuadraticCurve(5, 1, 20, 0.5)).to.be.equal(6.75);
    });
  });

  describe('round', () => {
    it('should stay the same on an integer', () => {
      expect(Math.round(3)).to.be.equal(3);
    });
    it('should round down below 0.5', () => {
      expect(Math.round(2.4999999)).to.be.equal(2);
    });

    it('should round up on 0.5', () => {
      expect(Math.round(2.5)).to.be.equal(3);
    });
  });
