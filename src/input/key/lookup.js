...
/**
 * Lookup covering possible keyCode to key. Format is `$keyCode + '-' + $keyLocation` 0 for `NONE`, 1 for `LEFT`, 2 for
 * `RIGHT`.
 *
 * **Input.KEY_LOOKUP:**
 * ```
 ...
 *
 * @param {String} key Internal key number to find a human readable key with
 * @returns {String|Boolean} Key we want to get based off of internal key number
 */
function lookup(key) {
  return LOOKUP[key]
    ? LOOKUP[key]
 ...
 *
 * @param {String} key Key we want to get the internal number for
 * @returns {String|Boolean} Internal key number
 */
